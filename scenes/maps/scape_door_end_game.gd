...
func _check_if_door_can_open():
	if is_door_open and play_door_anim:
		# If the animation has already been played before, just set it to last frame
		if play_door_anim.frame == play_door_anim.sprite_frames.get_frame_count("open") - 1:
			# Already at last frame, do nothing
			return
		play_door_anim.animation = "open"
		play_door_anim.frame = play_door_anim.sprite_frames.get_frame_count("open") - 1
		play_door_anim.stop()
...
func _on_escape_confirmed():
	if not is_door_open:
		...
		if play_door_anim:
			play_door_anim.animation = "open"
			play_door_anim.play() # ✅ First time open

...
func _ready():
	...
	# Hide and disable all loots
	call_deferred("_disable_all_loots")

	if chest_already_opened:
		chest_sprite.frame = chest_sprite.sprite_frames.get_frame_count("silver_chest_anim") - 1

	# If it's a temporary chest (no ID), add a 60s despawn timer
	if is_from_drone:
		var timer = Timer.new()
